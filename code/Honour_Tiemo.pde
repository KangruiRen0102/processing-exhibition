int stage = 0; // Current stage of metamorphosis
float progress = 0; // Transition progress
float caterpillarX = 50; // Initial position of the caterpillar
float butterflyX, butterflyY; // Position of the butterfly
boolean butterflyFlying = false; // Whether the butterfly is flying
float wingAngle = 0; // Wing flap angle
...
void drawCaterpillar(float x, float y) {
  for (int i = 0; i < 6; i++) {
...
void drawChrysalis(float x, float y) {
  fill(80, 150, 80);
  ellipse(x, y, 40, 60);
  stroke(50, 100, 50);
...
void drawEmergingButterfly(float x, float y, float scale) {
